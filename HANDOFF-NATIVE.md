...
| 4 — Food DB + recipes | ✅ **Shipped.** Search: `250e3b3` `/api/food-search` USDA proxy + `29c2891` `FoodSearchClient`/`FoodSearchSheet`. Recipes: `4fc462d` `Recipe` `@Model`+Schema, `RecipeBuilderSheet` (food-search `onPick` mode + manual ingredient), `RecipesView`+`LogRecipeSheet` (log N servings → scaled `MealLog`). Entry = Nutrition "+" menu. **⚠️ Needs `USDA_FDC_API_KEY` in Vercel env — search returns 503 until then.** |
| 5 — Apple Watch | ⬜ Queued. Separate watchOS target; needs user signing to run on device. |
| 6 — Strava/Oura/Weather | 🟡 **Server scaffolding shipped** (`a759ebf`): `/api/weather` (LIVE, keyless Open-Meteo), `/api/oura` (dormant, `OURA_PERSONAL_ACCESS_TOKEN`), `/api/strava/{auth/start,callback,sync}` (dormant, `STRAVA_CLIENT_ID/SECRET`; full OAuth, callback in middleware PUBLIC_PATHS, refresh-token persistence stubbed pending a schema column). **Remaining (needs user creds + device):** iOS Swift clients + a Settings "Integrations" connect UI — deliberately NOT built blind (would be dead code until the OAuth flow can be verified on-device with real credentials). |
