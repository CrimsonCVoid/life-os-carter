...
export function MacroRings() {
  ...
  const showNutritionOnToday = useStore((s) => s.settings.showNutritionOnToday);

  if (!showNutritionOnToday) return null;

  const hasTargets =
    !!macroTargets &&
    Object.values(macroTargets).some((v) => typeof v === "number" && v > 0);
  ...
  const bars = progress.filter((p) =>
    p.metric === "protein" || p.metric === "carbs" || p.metric === "fat"
  );
        ...
        <div className="flex-1 flex flex-col gap-3">
          {bars.map((b, i) => (
            <motion.div
              ...
              transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1], delay: 0.04 * i }}
            >
              <MacroBar item={b} />
            </motion.div>
          ))}
