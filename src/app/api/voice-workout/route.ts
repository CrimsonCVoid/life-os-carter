...
type SuccessResponse = { ok: true; parsed: ParsedSet[] };
type ErrorResponse = { ok: false; error: string };
export type VoiceWorkoutResponse = SuccessResponse | ErrorResponse;
...
- Match exerciseName to one of the KNOWN EXERCISES below if any name is close. If user says "bench" and "Bench press" is in known, use "Bench press" exactly.
- weight is in pounds, integer or 0 for bodyweight. If user says "kilos" or "kg", convert to pounds (× 2.20462) rounded to nearest 5.
- reps integer.
- rpe optional, 1-10.
  ...
  for (const item of list) {
    ...
    if (it.rpe != null) {
      ...
      if (Number.isFinite(r)) {
        const clamped = Math.max(1, Math.min(10, Math.round(r * 2) / 2));
        rpe = clamped;
      }
    }
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!apiKey) {
    return Response.json(
      {
        ok: false,
        error: `No Gemini API key found. Set one of: ${GEMINI_KEY_NAMES.join(", ")}.`,
  ...
  try {
    ...
    return Response.json(
      { ok: false, error: `Invalid request: ${msg}` } satisfies ErrorResponse,
      { status: 400 }
    );
  }
  ...
  try {
    ...
    return Response.json(
      { ok: false, error: "Couldn't decode audio." } satisfies ErrorResponse,
      { status: 400 }
    );
  }
  if (audioBuf.length === 0) {
    return Response.json(
      { ok: false, error: "Audio is empty." } satisfies ErrorResponse,
      { status: 400 }
    );
  }
  ...
  if (!parsedJson) {
    return Response.json(
      { ok: false, error: "Couldn't parse model output." } satisfies ErrorResponse,
      { status: 200 }
    );
  }
  ...
  return Response.json(
    { ok: true, parsed } satisfies SuccessResponse,
    { status: 200 }
  );
}
